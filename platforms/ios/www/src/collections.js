$(function () {
    ...
    window.entriesCollection = Backbone.Collection.extend({
        ...
        url: function () {
            if (this.loadDirection === "new") {
                ...
                return 'http://' + app.session.get("host") + '/resources/LiveDesk/Blog/' + app.session.get("blog") + '/Post/Published?cId.since=' + this.maxCid + '&asc=order&x-filter=*';
                ...
                return 'http://' + app.session.get("host") + '/resources/LiveDesk/Blog/' + app.session.get("blog") + '/Post/Published?limit=' + this.limit + '&cId.until=' + this.minCid + '&x-filter=*';
            } else {
                ...
                return 'http://' + app.session.get("host") + '/resources/LiveDesk/Blog/' + app.session.get("blog") + '/Post/Published?limit=' + this.limit + '&x-filter=*';
            }
        },
        parse: function (response) {
            console.log('parsing entriesList ...');
            return _.filter(response.PostList,
                    function (obj) {
                        if (!obj.DeletedOn && obj.IsPublished == 'True')
                            return obj;
                    });
        },
        /**
        ...
        updateCids: function () {
            ...
            pluckedCids = _.map(pluckedCids, function (num) {
                return parseInt(num);
            });
    ...
    window.postTypesCollection = Backbone.Collection.extend({
        model: PostTypeModel,
        url: function () {
            return 'http://' + app.session.get("host") + '/resources/my/LiveDesk/BlogType/' + app.session.get("blog") + '/Post?Authorization=' + app.session.get("session") + '&X-Filter=*';
        },
        parse: function (response) {
